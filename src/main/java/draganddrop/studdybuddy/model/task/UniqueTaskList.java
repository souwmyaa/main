...
public class UniqueTaskList implements Iterable<Task> {
    ...
    public void setTask(Task target, Task editedTask) {
        ...
        if (index == -1) {
            throw new draganddrop.studdybuddy.model.task.exceptions.TaskNotFoundException();
        }
    ...
    public void completeTask(Task target) {
        ...
        if (!internalList.contains(target)) {
            ...
            String finishDateTimeString = draganddrop.studdybuddy.logic.parser.TimeParser.getDateTimeString(LocalDateTime.now());
            target.setFinishDateTime(draganddrop.studdybuddy.logic.parser.TimeParser.parseDateTime(finishDateTimeString));
            int index = internalList.indexOf(target);
